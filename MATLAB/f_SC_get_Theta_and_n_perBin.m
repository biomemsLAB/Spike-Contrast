...
%           minBin:             minimal bin size of function "SpikeContrast"
...
function [AllSpikesPerBin,actElPerBin,edges,x]=f_SC_get_Theta_and_n_perBin(TS,time_start,time_end,bin,step)
    ...
    for n=1:size(TS,2)
       SPIKEHISTOGRAM(:,n)=f_SC_binning(nonzeros(TS(:,n)),time_start,time_end,bin,step,0);   % y_binned=binning(y,rec_dur,binsize,step,flag_binary)  
    end
